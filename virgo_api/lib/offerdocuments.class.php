...
class OfferDocuments{
	...
	protected static function BuildPhoto($row){
		$photo = new OfferDocument($row['id'],$row['offers_id'],$row['investments_id'],$row['filename'],$row['description'],$row['type'],$row['foto_id']);
		return $photo;
	}
	...
	*/
	...
	public static function GetPhoto($id){
		$result = DataBase::GetDbInstance()->ExecuteQueryWithParams("SELECT * FROM #S#offers_documents WHERE id=?", array((int) $id));
		if($result){
	 ...
	 */
	public static function AddPhoto(OfferDocument $photo){
		$query = "INSERT INTO #S#offers_documents (id, offers_id, investments_id, filename, description, `type`, foto_id) VALUES(?, ?, ?, ?, ?, ?, ?);";
        ...
        $params = array($get_id, $get_offer_id, $get_investment_id, (string)$photo->GetFilename(), (string)$photo->GetDescription(), $photo->GetType(), (int) $get_foto_id);
	 ...
	 * @param $photoNew
	 */
	public static function EditPhoto(OfferDocument $photoNew){
		$query = "UPDATE #S#offers_documents SET offers_id=?, filename=?, description=?, `type`=?, investments_id=?, foto_id=? WHERE id=?;";
		$params = array($photoNew->GetOfferId(), (string)$photoNew->GetFilename(),
			(string)$photoNew->GetDescription(), $photoNew->GetType(), $photoNew->GetInvestmentId(), $photoNew->GetFotoId(), (int) $photoNew->GetId());
		$result = DataBase::GetDbInstance()->ExecuteQueryWithParams($query, $params);
	}
	 ...
	 * @param OfferPhoto $photo
	 */
	public static function AddEditPhoto(OfferDocument $photo){	 
	...
	*/
	...
	public static function DeleteUnUsePhotos($offerId = false, $Ids = array(), $investmentId = false){
        ...
        public static function GetOfferPdf($url, $offerId, $langId){
            if (!is_dir($_SERVER['DOCUMENT_ROOT'] . Config::$AppPath. '/offers_pdf')){
                mkdir($_SERVER['DOCUMENT_ROOT'] . Config::$AppPath. '/offers_pdf');
            }
            if (!is_dir($_SERVER['DOCUMENT_ROOT'] . Config::$AppPath. '/offers_pdf/ofe_' . $offerId)){
                mkdir($_SERVER['DOCUMENT_ROOT'] . Config::$AppPath. '/offers_pdf/ofe_' . $offerId);
            }
            if (!is_dir($_SERVER['DOCUMENT_ROOT'] . Config::$AppPath. '/offers_pdf/ofe_' . $offerId . '/lng_' . $langId)){
                mkdir($_SERVER['DOCUMENT_ROOT'] . Config::$AppPath. '/offers_pdf/ofe_' . $offerId . '/lng_' . $langId);
            }
            $path = $_SERVER['DOCUMENT_ROOT'] . Config::$AppPath. '/offers_pdf/ofe_' . $offerId . '/lng_' . $langId . '/wydruk_' . $offerId . '.pdf';
            ...
            if (file_exists($path)){
                ...
                try{
                    ...
                    $buf = WebServiceWeb::WS()->GetPdfFromUrl($url);
        ...
        public static function DeleteOfferPdfFromDisk($idOffer){
            if($idOffer != 0){ 
                $path = $_SERVER['DOCUMENT_ROOT'].Config::$AppPath. '/offers_pdf/ofe_' . $idOffer;
                if (file_exists($path)) {                   
	 ...
	 * @return string
	 */  
	public static function GetImg(OfferDocument $photo, $customSize, $basicWatermark, $additionalWatermark, $kadruj){
		//testing size params
        ...
        $path = getcwd() . "/documents/ofs_" . $suf;
        if (!file_exists($path)) {mkdir($path);chmod($path, 0775);}
	
		$path = getcwd() . "/documents/ofs_$suf/offer_" . $photo->GetOfferId();
		if($photo->GetInvestmentId() > 0)
			$path = getcwd() . "/documents/investment_" . $photo->GetInvestmentId();
		if (!file_exists($path)) {mkdir($path);chmod($path, 0775);}
		...
		if (!file_exists($path)){
            ...
            $buf = WebServiceVirgo::WS()->GetClientDoc($photo->GetId());
			if ($buf != null) {
				$file = fopen($path, "wb");
				fwrite($file, $buf);
				fclose($file);
		...
		$path = Config::$AppPath . "/documents/ofs_$suf/offer_" . $photo->GetOfferId() . $fileName;
		if($photo->GetInvestmentId() > 0)
			$path = Config::$AppPath . "/documents/investment_" . $photo->GetInvestmentId() . $fileName;
		return $path;
	}
